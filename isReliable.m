function result = isReliable(mask,img)
...
fprintf('\n\t[isReliable] Selected pixels: %d\t Selected ratio: %4.2f\n ',selected_pixels,selected_pixels_ratio);
...
if result == true
    disp('   [isReliable] Result mask seems acceptable');
else
    if result_1 == false
        disp ('   [isReliable] Selected area is suspiciously big or small');
    end
    if result_2 == false
        disp('   [isReliable] Selected area is suspiciously unconnected..');
    end
end
